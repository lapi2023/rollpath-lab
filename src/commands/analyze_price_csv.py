...
def read_two_col_price_csv(path: Path) -> pd.DataFrame:
    """
    Read a two-column CSV where the left column is Date and the right column is Price.
    The loader is robust to header presence/names and simply uses the first two columns.
    Returns a DataFrame with columns ['Date', 'Price'] sorted by Date (ascending).
    """
    # Try with headers
    df = pd.read_csv(path)
    if df.shape[1] >= 2:
        ...
        # Fallback: headerless
        df = pd.read_csv(path, header=None, usecols=[0, 1], names=["Date", "Price"])
    ...
    if df.empty:
        raise ValueError("No valid rows after parsing Date/Price.")
...
def compute_simple_returns(df_price: pd.DataFrame) -> Dict[str, pd.Series]:
    ...
    # Monthly period returns (calendar month end)  # use "ME" instead of deprecated "M"
    price_m = df["Price"].resample("ME").last()
    r_m = price_m.pct_change(fill_method=None).dropna()

    # Yearly period returns (calendar year end)    # use "YE" instead of deprecated "Y"
...
def _hist(ax, data: pd.Series, bins: int, title: str):
    if data.empty:
        ...
        return
    b = int(bins)  # ensure integer for numpy.histogram
...
def plot_return_histograms(returns: Dict[str, pd.Series], title_prefix: str, outdir: Path, bins: int = 50, dpi: int = 150) -> Dict[str, Path]:
    ...
    paths = {}
    b = int(bins)  # normalize once here
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Load -> filter -> compute
    df_price = read_two_col_price_csv(csv_path)
