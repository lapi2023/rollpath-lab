"""
Path-related computations extracted from the original main.
Includes: typical normalized paths, value paths (DCA), and representative percentile paths.
"""
    ...
    *,
    ...
    for i in range(0, M, batch_size):
        ...
        if (style or "").lower() == "dca":
            ...
            C = inv_Pn * mask[None, :]
            C_prefix = np.cumsum(C, axis=1)
            V = Pn * (float(initial_cap) + float(amount) * C_prefix)
            values_all[i:j, :] = V
        else:
            V = Pn * init_eff
            values_all[i:j, :] = V
...
def build_representative_paths(
    ...
    then build full VALUE paths (with calendar dates). For DCA, also
    return the per-step Principal series used for stacked bars.
    """
    ...
    for lab in labels:
        ...
        if (style or "").lower() == "dca":
            eps = 1e-12
            inv_Pn = 1.0 / np.clip(Pn, eps, None)
            C = inv_Pn * base_mask.astype(np.float32)
            C_prefix = np.cumsum(C)
            principal = (float(initial_cap) + float(amount) * C_prefix).astype(np.float32)
            V = (Pn * principal).astype(np.float32)
            cmask = base_mask.copy()
        else:
            principal = np.full(W, init_eff, dtype=np.float32)
            V = (Pn * init_eff).astype(np.float32)
            cmask = None
